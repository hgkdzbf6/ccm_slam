...
//双目初始化
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, extractorptr pExtractorLeft, extractorptr pExtractorRight,vocptr pVoc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, size_t ClientId)
    :mpORBvocabulary(pVoc),mpORBextractorLeft(pExtractorLeft),mpORBextractorRight(pExtractorRight), mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mb(0), mThDepth(thDepth),
...
void Frame::ComputeStereoMatches()
{
    ...
    cout << "row indices: " << vRowIndices.size() << endl;
...
void Frame::trace(std::vector<std::vector<long unsigned int>> vec){
    for(auto it=vec.begin();it!=vec.end();it++){
